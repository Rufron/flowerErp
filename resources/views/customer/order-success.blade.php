...
<div class="bg-white shadow-lg rounded-2xl p-8 max-w-lg text-center mx-auto mt-20">
  ...
  <a href="{{ route('customer.shop') ?? '/shop' }}"
...
(function() {
    ...
    // IMPORTANT: Disable any cart restoration from order.js
    window.disableCartRestore = true;
    
    // Clear cart with extreme prejudice
    ...
    // Clear any backup/orders
    localStorage.removeItem('flower_orders_v1');
    
    // Override localStorage to prevent any script from restoring the cart
    const originalGetItem = Storage.prototype.getItem;
    Storage.prototype.getItem = function(key) {
        if (key === CART_KEY) {
            // Return empty array for any cart reads
            return '[]';
        }
        return originalGetItem.call(this, key);
    };
    
    // Override setItem to prevent cart writes
    const originalSetItem = Storage.prototype.setItem;
    Storage.prototype.setItem = function(key, value) {
        if (key === CART_KEY) {
            console.log('🚫 Blocked attempt to write to cart:', value);
            return; // Don't allow any cart writes
        }
    ...
    console.log('🎉 Cart clearing complete - all cart operations blocked');

    // Your existing order summary code...
    if (document.getElementById('order-summary').children.length === 0) {
        const params = new URLSearchParams(window.location.search);
        const orderId = params.get("orderId");
